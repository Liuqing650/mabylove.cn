...
import { Button ,Radio ,Icon,message,Input} from 'antd';
...
export default class Handsontable1 extends React.Component {
  ...
  this.setState({columnNum:col},function(){console.log('=====>',this.state.columnNum)})
  ...
  render() {
    return (<div>
      <Radio.Group>
      <Radio.Button value="large" onClick={this.onLoad}>加载</Radio.Button>
        <Radio.Button value="large" onClick={this.onClear}>清空</Radio.Button>
      <Radio.Button value="large" onClick={this.onSave}>保存</Radio.Button>
      <Radio.Button value="large" onClick={this.onCopy}>复制</Radio.Button>
        </Radio.Group>
        {this.state.columnNum}
      <div ref="table" style={{marginTop:10}} onChange={this.onHotDataChange}></div></div>)
  }
}
