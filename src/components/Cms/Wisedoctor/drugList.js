...
const pictureList = ({
  ...
  nextPic
    ...
    <Col span="4">
	<Card style={{ width: 120,cursor:"pointer"}} bodyStyle={{ padding: 0 }} onClick={()=>preview(index)} key={index}>
    <div style={{width:'100%',height:'80px',paddingLeft:'2%',paddingRight:'2%',
      paddingTop: '2%'}}>
      <div style={{backgroundSize:'cover',backgroundPosition:'center',backgroundImage:'url('+'http://tnfs.tngou.net/image'+item.img+')',height:'100%',width:'100%'}} >  
      </div>
    </div>
  ...
  <Row style={{marginBottom:20}}>
    ...
    <Switch onChange={changeSwitch} checkedChildren={'图片'} unCheckedChildren={'表格'}  style={{float:'right'}}/>
  </Row>
    {controller&&
	  <div>{imageArray}
    <Modal visible={previewVisible} footer={null} onCancel={handleCancel} maskClosable={false}>
    <div className={styles.container}>
        <a onClick={prePic} className={classNames(styles.arrow,styles.arrow_left)}>&lt;</a>
        <a onClick={nextPic} className={classNames(styles.arrow,styles.arrow_right)}>&gt;</a>
