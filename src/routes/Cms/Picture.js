...
function Picture({location,dispatch,picture}) {
    ...
    preview(index){
      ...
      dispatch({type:'picture/querySuccess',payload:{previewImage:url,previewVisible:true,index:index}})
    },
    handleCancel(){
      dispatch({type:'picture/querySuccess',payload:{previewVisible:false}})
    },
    prePic(){
      if(index==0){
      message.info('已经是第一张')
      ...
      dispatch({type:'picture/querySuccess',payload:{previewImage:url,index:index-1}})
    },
    nextPic(){
