...
function Microapp({location,dispatch,picture}) {
  ...
  const qrcodeProps={
    ...
    changeValue(e){
      dispatch({type:'picture/querySuccess',payload:{value:e.target.value}})
    },
    changeBgColor(e){
      dispatch({type:'picture/querySuccess',payload:{bgColor:e.target.value}})
    },
    changeFgColor(e){
      dispatch({type:'picture/querySuccess',payload:{fgColor:e.target.value}})
    },
    changeLevel(value){
      dispatch({type:'picture/querySuccess',payload:{level:value}})
    },
    onReload(){
      dispatch({type:'picture/querySuccess',payload:{
        size:128,
  ...
  return (
    ...
    <TabPane tab={<span><Icon type="clock-circle" />年龄</span>} key="1"><Age/></TabPane>
    <TabPane tab={<span><Icon type="calculator" />健康</span>} key="2"><Bmi/></TabPane>
    <TabPane tab={<span><Icon type="pay-circle" />工资</span>} key="3"><Salary/></TabPane>
    <TabPane tab={<span><Icon type="bank" />房租</span>} key="4"><House/></TabPane>
    <TabPane tab={<span><Icon type="qrcode" />二维码</span>} key="5"><QRcode {...qrcodeProps}/></TabPane>
    <TabPane tab={<span><Icon type="cloud-o" />天气</span>} key="6"><Weather {...weatherProps}/></TabPane>
    </Tabs>
    </div>
  )
}
