...
function MenuManage({ location, dispatch, menuManage }) {
  const {
    loading,menuListData,modalVisible,modalType,currentItem
    } = menuManage 
    const menuManageListProps={ 
      ...
      changeState(value,menu_id){
        ...
        dispatch({type:'menuManage/changeState',payload:{state:state,menu_id:menu_id}})
      },
      deleteMenu(value){
        dispatch({type:'menuManage/deleteMenu',payload:{menu_id:value}})
      },
      addMenu(record){
        dispatch({type:'menuManage/querySuccess',payload:{currentItem:record,modalVisible:true,modalType:'add'}})
      },
      editMenu(record){
        dispatch({type:'menuManage/querySuccess',payload:{currentItem:record,modalVisible:true,modalType:'edit'}})
      }
    }
    const menuManageModalProps={
      ...
      onCancel(){
        dispatch({type:'menuManage/querySuccess',payload:{modalVisible:false}})
      },
      onOk(data){
        if(modalType=='add'){
        dispatch({type:'menuManage/addMenu',payload:{
        ...
        dispatch({type:'menuManage/updateMenu',payload:{
          menu_name:data.menu_name,
          code:data.code,
          src:data.src,
